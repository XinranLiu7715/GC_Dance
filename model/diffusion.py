...
from vis import skeleton_render, process_dataset
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, cond, t, text,  weight=None, clip_x_start = False):
        weight = weight if weight is not None else self.guidance_weight
        model_output = self.model.guided_forward(x, cond, t, text, weight)
    ...
    def p_mean_variance(self, x, cond, t, text):
        # guidance clipping
        ...
        x_recon = self.predict_start_from_noise(
            x, t=t, noise=self.model.guided_forward(x, cond, t, text, weight)
        )
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, text,):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, cond=cond, t=t, text = text
        )
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, motion, cond,  text, **kwargs):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(x, cond, time_cond,  text, clip_x_start = self.clip_denoised)
    ...
    @torch.no_grad()
    def long_ddim_sample(self, shape, cond, text, **kwargs):
        batch, device, total_timesteps, sampling_timesteps, eta = shape[0], self.betas.device, self.n_timestep, 50, 1
        ...
        for time, time_next, weight in tqdm(time_pairs, desc = 'sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            pred_noise, x_start, *_ = self.model_predictions(x, cond, time_cond, text, weight=weight, clip_x_start = self.clip_denoised) 
    ...
    def inpaint_loop(
        ...
        if return_diffusion:
            diffusion = [x]
        constraint = {}
        # 创建一个 mask，将前 50 个时间步固定
        constraint["mask"] = torch.zeros((x.shape[0], x.shape[1], x.shape[2]))
        constraint["mask"][:, :, :] = 1
        constraint["mask"][:, :, :8] = 0  # 前 50 步设置为 1，表示要固定
        constraint["mask"][:, :, 9*6:11*6] = 0 
        constraint["mask"][:, :, 12*6:14*6] = 0 
        constraint["mask"][:, :, 15*6:17*6] = 0 
        constraint["mask"][:, :, 18*6:20*6] = 0 

        # 定义 value，比如这里我们可以用一个全零张量作为固定的值
        constraint["value"] = motion
        ...
        for i in tqdm(reversed(range(0, start_point))):
            ...
            x, _ = self.p_sample(x, cond,timesteps, text)
    ...
    def p_losses(self, x_start, cond, t, text):
        noise = torch.randn_like(x_start)
        ...
        _, target_ = torch.split(target, (4, target.shape[2] - 4), dim=2)
    ...
    def loss(self, x, cond, text, t_override=None):
        ...
        return self.p_losses(x, cond, t, text)

    def forward(self, x, cond, text, t_override=None):
        return self.loss(x, cond, text, t_override)
    ...
    def render_sample(
        ...
        if samples.shape[2] == 319 or samples.shape[2] == 151 or samples.shape[2] == 139:
        ...
        if mode == "long":
            ...
            b,s,_ = model_q.shape
            if self.opt.nfeats == 139 or self.opt.nfeats==135:
                poses = self.smplx_fk.forward(model_q, model_x)
            else:
                model_q = model_q.view(b, -1)
                model_x = model_x.view(-1, 3)
                
                poses = self.smplx_fk.forward(model_q, model_x)
                full_pose = poses.view(b, -1, 3).detach().cpu().numpy()
            ...
            if fk_out is not None:
                ...
                pickle.dump(
                    {
                        "smpl_poses": full_q.squeeze(0).reshape((-1, reshape_size)).cpu().numpy(),
                        "smpl_trans": full_pos.squeeze(0).cpu().numpy(),
                        "full_pose": full_pose,                                          # 3d positions
                    },
        ...
        if self.opt.nfeats == 139 or self.opt.nfeats==135:
            ...
            model_x = model_x.view(-1, 3)
            poses = self.smplx_fk.forward(model_q, model_x)
            poses = poses.view(b, s, -1, 3).detach().cpu().numpy()
