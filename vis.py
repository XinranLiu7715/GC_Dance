...
def process_dataset(root_pos, local_q):
        ...
        pos_rotation = RotateAxisAngle(90, axis="X", degrees=True).to(root_pos.device)
...
def plot_single_pose(num, poses, lines, ax, axrange, scat, contact, ske_parents):
    ...
    for i, (p, line) in enumerate(zip(ske_parents, lines)):
        ...
        #color = 'orange' if i < 12 else 'blue' 
...
def skeleton_render(
    ...
    if sound:
        ...
        if render:
            ...
            #temp_dir.name = '/vol/research/CMVCG/xl/code/FineDance/test/render'
...
class SMPLX_Skeleton:
    def __init__(
        ...
        self.J = np.load("assets/smplx_neu_J_1.npy")
