...
class NashMTL(WeightMethod):
    ...
    def solve_optimization(self, gtg: np.array):
        ...
        for _ in range(self.optim_niter):
            alpha_t = np.nan_to_num(alpha_t, nan=0.0, posinf=1e5, neginf=0.0)
        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            trainable_shared_parameters = [
                p for p in shared_parameters if p.requires_grad
            ]
            grads = {}
            for i, loss in enumerate(losses):
                ...
                #unused_params = [(param, grad) for param, grad in zip(shared_parameters, g) if grad is None]
                # 获取模型的所有命名参数
                #param_to_name = {id(param): name for name, param in model.named_parameters()}

                # 打印未使用的参数及其所属模块
                #print("\n=== Unused Parameters and Their Sources ===")
                #for i, (param, _) in enumerate(unused_params):
                #    param_id = id(param)
                #    param_name = param_to_name.get(param_id, "Unknown")
                #    print(f"Unused Parameter {i}: Name = {param_name}, Shape = {param.shape}, Norm = {param.norm().item():.4f}")
